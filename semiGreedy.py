# import
from config import args, T, N, Q, b, S1, S2, SP, DH, r, gamma, C, P, periods, groups, teams, intervals
...
# ----------------------------------------------------------------------------------------------------------------------
# Function(s) implementing the semi-greedy used to build an initial feasible solution
# ----------------------------------------------------------------------------------------------------------------------
def srptSort(interval, group_remaining_processing_time):
    # Define 'outlook' (i.e., parameter that indicates the number of time intervals ahead to consider for the next reading) -- HARDCODED
    OUTLOOK = 24

    # Initialise 'group_order'
    group_order = -1 * np.ones(len(groups), dtype=int)

    # Initialise 'group_ordered'
    group_ordered = np.zeros(len(groups), dtype=int)
    ...
    group_next_reading = -1 * np.ones(len(groups), dtype=int)
...
def greedyRandomizedConstructiveHeuristics():
    ...
    group_processing_time = np.array([m.ceil(N[j] / Q) for j in groups])
    ...
    group_completed = np.zeros(len(groups), dtype=int)

    # Initialise 'completion time'
    group_completion_time = -1 * np.ones(len(groups), dtype=int)
    ...
    while interval < len(intervals):
        ...
        group_candidate_list = -1 * np.ones(len(groups), dtype=int)

        # Initialise 'group_selected'
        group_selected = -1 * np.ones(len(groups), dtype=int)

        # Append all the indexes of the meter groups that can be assigned to teams to 'group_candidate_list' in ascending order based on the shortest remaining processing time, breaking ties by farthest next reading
        for j in groups:
        ...
        # Initialise 'team_assignment'
        team_assignment = np.zeros(len(teams), dtype=int)
        ...
        for k in teams:
            ...
            while len(group_restricted_candidate_list) < args.argAlpha and group_index < len(groups):
            ...
            # If 'group_restricted_candidate_list' is not empty and team 'k' is idle
            if group_restricted_candidate_list and team_assignment[k] == 0:
                ...
                # If the remaining number of meters is greater than or equal to the capacity
                ...
                # If the remaining number of meters is less than the capacity
                else:
                    # Update 'x'
    ...
    for j in groups:
        ...
        assert t >= 0, f"Meter group {j} was never completed within the operational horizon."
    ...
    for p in range(SP):
        start_interval = T * p
        end_interval = T * (p + 1)
        solution.S[p] = sum(S1[j][t] * solution.z[j, t] for j in groups for t in range(start_interval, end_interval))

    # Update 'S(p)' afterward
    for p in range(SP, P - 1):
        solution.S[p] = sum(S2[j] for j in groups)

    # Update 'X(p)' within the operational horizon
    for p in range(SP):
        start_interval = T * p
        end_interval = T * (p + 1)
        solution.X[p] = sum(C * solution.x[j, t] for j in groups for t in range(start_interval, end_interval))
    ...
    for p in range(3, P + 1):
        start_varphi = 0
        end_varphi = (p - 2) + 1
        solution.R[p] = solution.D[p - 2] + r * sum(solution.X[varphi] - solution.D[varphi] for varphi in range(start_varphi, end_varphi))
